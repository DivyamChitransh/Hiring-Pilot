...
export const getAllCompanies = async (req: AuthRequest,res: Response) => {
  try {
    ...
    return res.status(200).json({success: true,message: "Companies fetched successfully",
      data: companies.data,pagination: {page,limit,total: companies.total,totalPages: Math.ceil(companies.total / limit)}
    });
  } catch (error) {
